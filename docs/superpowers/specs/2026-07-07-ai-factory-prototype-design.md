...
| Agents / Agent Registry | 确认 Agent 能力和权限边界 | backend_coder、review_agent、能力、限制、审计 | 查看 Agent 配置和历史记录 |
| Architecture / Architecture Trace | 解释系统链路、状态和可信度 | 组件链路、FactoryState、事件审计、错误恢复 | 定位失败节点、查看事件详情 |
